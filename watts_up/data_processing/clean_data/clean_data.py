...
def clean_price_data():
    ...
    for index, row in cleaned_df.iterrows():
        ...
        cleaned_df.at[index, "year_state"] = (
            f"{cleaned_df.at[index, 'year']}_{row['stateid']}"
        )
    ...
    df_pivoted = cleaned_df.pivot_table(
        index=["stateid", "year", "year_state"], columns="sectorid", values="price"
    ).reset_index()
    df_pivoted.rename(
        columns={
            "ALL": "price_all",
...
def clean_gdp_data():
    ...
    merged_df = (
        pd.merge(
            melted_df, STATE_MAPPING, left_on="State", right_on="state", how="left"
        )
...
def clean_pop_data():
    ...
    merged_df = pd.merge(
        pop_2019, pop_2022, left_on="state", right_on="state", how="left"
    )
    merged_df = merged_df[merged_df["state"] != "Puerto Rico"]
    pop_final = pd.melt(
        merged_df, id_vars=["state"], var_name="year", value_name="population"
    )
    merged_df1 = pd.merge(
        pop_final, STATE_MAPPING, left_on="state", right_on="state", how="left"
    )
    merged_df1["year_state"] = (
        merged_df1["year"].astype(str) + "_" + merged_df1["stateid"].astype(str)
    )
